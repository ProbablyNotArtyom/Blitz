 ...
 *
 * $Id: alg_info.c,v 1.2 2002/07/24 07:41:01 philipc Exp $
 *
 ...
 */
static int
enum_search_prefix (enum_names *ed, const char *prefix, const char *str, int strlength)
{
	char buf[64];
	char *ptr;
	int ret;
 ...
 */
static int
enum_search_ppfix (enum_names *ed, const char *prefix, const char *postfix, const char *str, int strlength)
{
	char buf[64];
	char *ptr;
	int ret;
...
enum_search_prefix (enum_names *ed, const char *prefix, const char *str, int strlength)
{
	...
	while (strlength--&&len--&&*str) *ptr++=toupper(*str++);
	*ptr=0;
	DBG(DBG_CRYPT, DBG_log(__FUNCTION__ "() "
				"calling enum_search(%p, \"%s\")", ed, buf));
	ret=enum_search(ed, buf);
