...
#if (defined(CONFIG_UCSIMM) || defined(CONFIG_UCDIMM) || defined(CONFIG_DRAGEN2)|| defined(CONFIG_CWEZ328) || defined (CONFIG_CWVZ328)) && !DEBUG
...
void kfree (const void *objp)
{
	...
	if (!PageSlab(p)) {
		if (objp != page_address(p)) {
			printk("flgs=0x%x objp=0x%x pga=0x%x\n", p->flags, objp, page_address(p));
			BUG();
		}
		if (!page_contig(p)) {
			printk("flgs=0x%x objp=0x%x pga=0x%x\n", p->flags, objp, page_address(p));
		...
		free_contiguous_pages((unsigned long)objp, page_contig(p));
		local_irq_restore(flags);
		return;
	}
#endif
