...
/* This makes an entire environment with or without the unexported
 * entries.
 */
static char **
makefullenv(allp)
...
int quoted;
{
	...
	av[0] = "-sh";
	av[1] = "-c";
	av[2] = e.iop->argp->aword;
	*cp = '\0';
	av[3] = NULL;
	/* Create an environment for the subprocess.  This should include
	 * all parameters including those not exported.
	 */
	environ = makefullenv(1);
#endif
	if ((i = vfork()) == -1) {
#ifdef EMBED
		*cp = '`';
	...
	if (i != 0) {
#ifdef EMBED
		*cp = '`';	/* Okay since child has already execed */
#endif
		e.iop->argp->aword = ++cp;
		close(pf[1]);
