...
/* Default the login bin path if not defined. */
...
#ifdef EMBED
#define DEF_ERASE	CTL('H')	/* default erase character */
#else
...
#elif defined(__UC_LIBC__) || defined(__USE_GNU)
		    (void) printf ("%s", uts.domainname);
#else
		    (void) printf ("%s", uts.__domainname);
#endif
		    break;
